...
if [ "$volume" = "0" ]; then
        icon_name="/usr/share/icons/Suru++/status/48/audio-volume-muted.svg"
$DIR/notify-send.sh "$volume""      " -i "$icon_name" -t 2000 -h int:value:"$volume" -h string:synchronous:"─" --replace=555
    else
	if [  "$volume" -lt "10" ]; then
	     icon_name="/usr/share/icons/Suru++/status/48/audio-volume-low.svg"
$DIR/notify-send.sh "$volume""     " -i "$icon_name" --replace=555 -t 2000
        ...
        if [ "$volume" -lt "30" ]; then
            ...
            if [ "$volume" -lt "70" ]; then
                icon_name="/usr/share/icons/Suru++/status/48/audio-volume-medium.svg"
            else
                icon_name="/usr/share/icons/Suru++/status/48/audio-volume-high.svg"
            fi
        fi
    fi
fi
